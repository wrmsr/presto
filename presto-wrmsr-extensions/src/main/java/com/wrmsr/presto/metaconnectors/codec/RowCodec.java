...
/**
 * Implementations decode a  message from bytes and add field value providers for all decodable columns.
 */
public interface RowCodec
{
     ...
     *
     * @param data The row data ( message) to decode.
     ...
     * @return false if the row was decoded successfully, true if it could not be decoded (was corrupt). TODO - reverse this boolean.
     */
    boolean decodeRow(
            byte[] data,
            Set<FieldValueProvider> fieldValueProviders,
            List<CodecColumnHandle> columnHandles,
            Map<CodecColumnHandle, FieldCodec<?>> fieldDecoders);
}
