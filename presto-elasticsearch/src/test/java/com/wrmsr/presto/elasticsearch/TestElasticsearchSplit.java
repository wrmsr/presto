...
{
    private final ElasticsearchSplit split = new ElasticsearchSplit("connectorId", "schemaName", "tableName", URI.create("http://127.0.0.1/test.file"));

    @Test
    public void testAddresses()
    {
        // http split with default port
        ElasticsearchSplit httpSplit = new ElasticsearchSplit("connectorId", "schemaName", "tableName", URI.create("http://example.com/example"));
        ...
        httpSplit = new ElasticsearchSplit("connectorId", "schemaName", "tableName", URI.create("http://example.com:8080/example"));
        assertEquals(httpSplit.getAddresses(), ImmutableList.of(HostAddress.fromParts("example.com", 8080)));
        assertEquals(httpSplit.isRemotelyAccessible(), true);

        // http split with default port
        ElasticsearchSplit httpsSplit = new ElasticsearchSplit("connectorId", "schemaName", "tableName", URI.create("https://example.com/example"));
        ...
        // http split with custom port
        httpsSplit = new ElasticsearchSplit("connectorId", "schemaName", "tableName", URI.create("https://example.com:8443/example"));
    ...
    {
        JsonCodec<ElasticsearchSplit> codec = jsonCodec(ElasticsearchSplit.class);
        String json = codec.toJson(split);
        ElasticsearchSplit copy = codec.fromJson(json);
