...
public class NestedJarPath
        implements Path
{
    private final NestedJarFileSystem fileSystem;
    private final byte[] path;
    private volatile int[] offsets;
    private volatile int hash = 0;
    private volatile byte[] resolved = null;

    public NestedJarPath(NestedJarFileSystem fileSystem, byte[] path)
    {
        this(fileSystem, path, false);
    }

    public NestedJarPath(NestedJarFileSystem fileSystem, byte[] path, boolean normalized)
    {
    ...
    public NestedJarFileSystem getFileSystem()
    {
        return fileSystem;
    }
    ...
    public NestedJarPath getRoot()
    {
        if (isAbsolute()) {
            return new NestedJarPath(fileSystem, new byte[] {this.path[0]});
        }
        return null;
    }

    public NestedJarPath getFileName()
    {
        initOffsets();
        ...
        return new NestedJarPath(fileSystem, path);
    }

    public NestedJarPath getParent()
    {
        initOffsets();
        int nbOffsets = offsets.length;
        if (nbOffsets == 0) {
    ...
    public NestedJarPath getName(int index)
    {
        initOffsets();
    ...
    public NestedJarPath subpath(int beginIndex, int endIndex)
    {
        initOffsets();
    ...
    {
        NestedJarPath p1 = this;
        NestedJarPath p2 = checkPath(other);
    ...
    public NestedJarPath normalize()
    {
        byte[] p = getResolved();
        if (p == this.path) {
            return this;
        }
        return new NestedJarPath(fileSystem, p, true);
    }

    public NestedJarPath resolve(Path other)
    {
        NestedJarPath p1 = this;
        NestedJarPath p2 = checkPath(other);
        if (p2.isAbsolute()) {
        ...
        return new NestedJarPath(fileSystem, result);
    }

    public NestedJarPath resolve(String other)
    {
    ...
    public Path resolveSibling(Path other)
    {
        ...
        NestedJarPath parent = getParent();
    ...
    public NestedJarPath relativize(Path other)
    {
        NestedJarPath p1 = this;
        NestedJarPath p2 = checkPath(other);
        if (p2.equals(p1)) {
            return new NestedJarPath(fileSystem, new byte[0], true);
        }
    ...
    public NestedJarPath toAbsolutePath()
    {
        if (isAbsolute()) {
            return this;
        }
        ...
        return new NestedJarPath(fileSystem, result, true);
    }

    public NestedJarPath toRealPath(LinkOption... options)
            throws IOException
    {
        NestedJarPath absolute = new NestedJarPath(fileSystem, getResolvedPath()).toAbsolutePath();
        ...
        return new Iterator<Path>()
        {
            ...
            public Path next()
            {
                if (index < getNameCount()) {
                    NestedJarPath name = getName(index);
                    index++;
                    return name;
                }
    ...
    {
        NestedJarPath p1 = this;
        NestedJarPath p2 = checkPath(paramPath);
        byte[] a1 = p1.path;
        byte[] a2 = p2.path;
        int l1 = a1.length;
        int l2 = a2.length;
    ...
    private NestedJarPath checkPath(Path paramPath)
    {
        if (paramPath == null) {
            throw new NullPointerException();
        }
        if (!(paramPath instanceof NestedJarPath)) {
            throw new ProviderMismatchException();
        }
        return (NestedJarPath) paramPath;
    }
    ...
    {
        return obj instanceof NestedJarPath
                && ((NestedJarPath) obj).fileSystem == fileSystem
                && compareTo((NestedJarPath) obj) == 0;
    }
    ...
    private static byte[] doGetResolved(NestedJarPath p)
    {
    ...
    private static boolean equalsNameAt(NestedJarPath p1, NestedJarPath p2, int index)
    {
