...
public class NestedJarFileSystem
        extends FileSystem
{
    private final NestedJarFileSystemProvider fileSystemProvider;
    ...
    public NestedJarFileSystem(NestedJarFileSystemProvider fileSystemProvider, String repository, Map<String, ?> env)
            throws IOException
    {
        String userInfo;
        String query;
    ...
    {
        return Collections.singleton(new NestedJarPath(this, new byte[] {'/'}));
    }
    ...
    public Path getPath(String first, String... more)
    {
        ...
        return new NestedJarPath(this, path.getBytes(StandardCharsets.UTF_8));
    }
    ...
    public DirectoryStream<Path> newDirectoryStream(final Path dir, DirectoryStream.Filter<? super Path> filter)
            throws IOException
    {
            ...
            public Iterator<Path> iterator()
            {
                return new Iterator<Path>()
                {
                    ...
                    public Path next()
                    {
                        ...
                        return new NestedJarPath(NestedJarFileSystem.this, ((String) val.get("path")).getBytes(StandardCharsets.UTF_8));
                    }
    ...
    public <A extends BasicFileAttributes> SeekableByteChannel newByteChannel(Path path, Set<? extends OpenOption> options, FileAttribute<?>[] attrs)
            throws IOException
    {
        Object content = loadContent(path.toAbsolutePath().toString());
        if (content instanceof List) {
    ...
    public <A extends BasicFileAttributes> A readAttributes(Path path, Class<A> clazz, LinkOption... options)
            throws IOException
    {
        ...
        return (A) new NestedJarFileAttributes(type, size);
    }

    private Object loadContent(String path)
            throws IOException
    {
    ...
    private static class NestedJarFileAttributes
            implements BasicFileAttributes
    {
        ...
        private NestedJarFileAttributes(String type, long size)
        {
            this.type = type;
            this.size = size;
        }
