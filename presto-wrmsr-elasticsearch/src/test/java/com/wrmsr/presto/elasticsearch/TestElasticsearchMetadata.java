...
public class TestElasticsearchMetadata
{
    ...
    private static final ElasticsearchTableHandle NUMBERS_TABLE_HANDLE = new ElasticsearchTableHandle(CONNECTOR_ID, "example", "numbers");
    ...
    public void testGetColumnHandles()
    {
        ...
        assertEquals(metadata.getColumnHandles(NUMBERS_TABLE_HANDLE), ImmutableMap.of(
                "text", new ElasticsearchColumnHandle(CONNECTOR_ID, "text", VARCHAR, 0),
                "value", new ElasticsearchColumnHandle(CONNECTOR_ID, "value", BIGINT, 1)));

        // unknown table
        try {
            metadata.getColumnHandles(new ElasticsearchTableHandle(CONNECTOR_ID, "unknown", "unknown"));
            fail("Expected getColumnHandle of unknown table to throw a TableNotFoundException");
        }
        catch (TableNotFoundException expected) {
        }
        try {
            metadata.getColumnHandles(new ElasticsearchTableHandle(CONNECTOR_ID, "example", "unknown"));
            fail("Expected getColumnHandle of unknown table to throw a TableNotFoundException");
        }
        catch (TableNotFoundException expected) {
        }
    ...
    public void getTableMetadata()
    {
        ...
        assertNull(metadata.getTableMetadata(new ElasticsearchTableHandle(CONNECTOR_ID, "example", "unknown")));
        assertNull(metadata.getTableMetadata(new ElasticsearchTableHandle(CONNECTOR_ID, "unknown", "numbers")));
    }
