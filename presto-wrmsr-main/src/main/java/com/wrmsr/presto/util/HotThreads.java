...
public class HotThreads {
    ...
    private Duration interval = new Duration(500, TimeUnit.MILLISECONDS);
    private Duration threadElementsSnapshotDelay = new Duration(10, TimeUnit.SECONDS);
    private int threadElementsSnapshotCount = 10;
    private String type = "cpu";
    private boolean ignoreIdleThreads = true;

    public HotThreads interval(Duration interval) {
    ...
    public HotThreads threadElementsSnapshotDelay(Duration threadElementsSnapshotDelay) {
    ...
    private String innerDetect() throws Exception {
        ...
        try {
            ...
            // skip that for now
            hotties.sort(new Comparator<HotThreads.MyThreadInfo>() {
                @Override
                public int compare(HotThreads.MyThreadInfo o1, HotThreads.MyThreadInfo o2)
                {
                    if ("cpu".equals(type)) {
            ...
            for (int j = 0; j < threadElementsSnapshotCount; j++) {
                ...
                Thread.sleep(threadElementsSnapshotDelay.toMillis());
            }
            for (int t = 0; t < busiestThreads; t++) {
                ...
                double percent = (((double) time) / interval.getValue(TimeUnit.NANOSECONDS)) * 100;
                sb.append(String.format(Locale.ROOT, "%n%4.1f%% (%s out of %s) %s usage by thread '%s'%n", percent, new Duration(time, TimeUnit.NANOSECONDS), interval, type, threadName));
